...
/-! Complete quinary cut coverage assembled from numeric mask checks. -/
namespace AMEProbe.QuinaryDirect
open AMEProbe.MaskEncoding

private theorem certificate_of_lookup (A : Finset (Fin 12)) (hA : A.card = 6)
    (b : ℕ) (lookup : ℕ → {A : Finset (Fin 12) // Certificate A})
    (hdiv : mask A / 256 = b)
    (hmask : ∀ r : Fin 256, (256 * b + r.val).bitIndices.length = 6 →
  ...
  have hdecomp : 256 * b + r.val = mask A := by
    ...
    rw [hdiv] at hm
    omega
  have hbits : (mask A).bitIndices.length = 6 := by
    change (finMask A).bitIndices.length = 6
...
theorem all_certified (A : Finset (Fin 12)) (hA : A.card = 6) : Certificate A := by
  obtain ⟨b, hb⟩ := batch_coverage A hA
  fin_cases b
  · exact Batch00.checked A hb
...
#print axioms AMEProbe.QuinaryDirect.all_certified
